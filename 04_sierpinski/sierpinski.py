...
def main():
	"""
	Let's create a sierpinski triangle.
	"""
...
def sierpinski_triangle(order, length, upper_left_x, upper_left_y):
	...
	if order == 0:
		...
		window.add(triangle)
		# Upper left mini triangle
		sierpinski_triangle(order - 1, length / 2, upper_left_x, upper_left_y)
		# Upper right mini triangle
		sierpinski_triangle(order - 1, length / 2, upper_left_x+length / 2, upper_left_y)
		# Bottom mini triangle
		sierpinski_triangle(order - 1, length / 2, upper_left_x + length/2/2, upper_left_y+length*0.866/2)
